...
public class Sort {
     ...
     * 삽입정렬
     *
     * 배열의 모든 요소를 앞에서부터 차례대로 이미 정렬된 배열 부분과 비교하여 위치를 삽입
     * - 배열이 길어질수록 효율이 떨어짐
     *
     * 1. 두 번째 원소부터 시작하여 부분 배열에서 적절한 위치에 삽입(찾아서 교환)
     *      - 현재 원소부터 이전 원소들을 순환하면서 값이 작다면 0번지 까지 교환작업 반복
     * 2. 세 번째 원소를 부분 배열에서 적절한 위치에 삽입(앞에서 i+1 index 만큼의 배열에서 찾아서 교환)
     * 3. 위의 경우 반복
     *
     * @param arr
     */
